...
The `create_filters` function produces a collection of objects used by
the `query` method to generate a stream of `CloseApproach` objects that
match all desired criteria. The arguments to `create_filters` are provided
by the main module and originate from the user's command-line options.
...
class AttributeFilter:
    """A superclass for filters on comparable attributes.
    ...
    Subclasses can override the `get` classmethod to provide custom
    ...
    def __repr__(self):
        """Return a string representation of the AttributeFilter instance."""
...
class DiameterFilter(AttributeFilter):
    """Filter close approaches based on the NEO's diameter."""
    @classmethod
    def get(cls, approach):
...
class HazardousFilter(AttributeFilter):
    """Filter close approaches based on whether the NEO is hazardous."""
    @classmethod
    def get(cls, approach):
...
def create_filters(
    ...
    that occurred on exactly that given date. Similarly, the `--min-distance`
...
def limit(iterator, n=None):
    ...
    """
    # If n is None or 0, return the entire iterator.
    if n is None or n == 0:
        return iterator
    # Otherwise, use islice to yield at most n values from the iterator.
